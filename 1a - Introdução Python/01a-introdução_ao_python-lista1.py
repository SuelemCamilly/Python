...
def dias_perdidos_por_fumar(cigarros_fumados_por_dia, anos_fumando):
    ...
    """
    return int((cigarros_fumados_por_dia * (anos_fumando * 365)) * 10 / 60 / 24)
...
def media_final_aprovado_reprovado(p1, p2, ep1, ep2):
    ...
    """
    if (p1 * 7 + ep1 * 3) / (7 + 3) + (p2 * 7 + ep2 * 3) / (7 + 3) / 2 >= 7:
        return True
...
def salario(valor_hora, horas_mensais):
    ...
    descontos = (salario * 8 / 100) + (salario * 11 / 100) + (salario * 5 / 100)
    return float(salario - descontos)
...
def tinta(metros_pintar):
    ...
    """
    return ceil(metros_pintar / 3 / 18)
...
def decompor_numero(n):
    ...
    n = (n - dezena) / 10
    centena = n
    return (int(centena), int(dezena), unidade)
